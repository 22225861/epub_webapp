...
export default {
  ...
  mounted () {
  //  this.currentBook.ready.then(() => { this.doSearch('add').then(list => { this.searchList = list }) })
  },
  methods: {
    ...
    displayContent (target, highlight = false) {
      this.display(target, () => {
        ...
        if (highlight) {
          this.currentBook.rendition.annotations.highlight(target) // 实现跳转之后页面的根据输入框内容高亮显示
        }
      })
    }
  }
